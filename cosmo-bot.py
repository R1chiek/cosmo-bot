...
async def main():
    ...
    async with client:
        try:
            source_link = source_chat_urls[0]
            if "t.me" in source_link:
                username, message_id = parse_message_link(source_link)
                source_chat = await client.get_entity(username)
            else:
                source_chat = await client.get_entity(source_link)
                message_id = photo_message_id  # Используем стандартный ID фото
        except errors.ChannelPrivateError:
            logging.info(f'Не удалось получить доступ к исходному чату {source_link}.')
            ...
            for target_chat_url in target_chat_urls:  # Внутренний цикл перебора чатов
                try:
                    ...
                    try:
                        ...
                        logging.info(f'Сообщение с ID {message_id} успешно отправлено в {target_chat.title}.')
