...
async def main():
    ...
    async with client:
        try:
            ...
            print(f'Не удалось получить доступ к исходному чату {source_chat_urls[0]}.')
            ...
            for target_chat_url in target_chat_urls:  # Внутренний цикл перебора чатов
                try:
                    ...
                    try:
                        ...
                        print(f'Фото с ID {photo_message_id} успешно отправлено в {target_chat.title}.')
                    except errors.SlowModeWaitError as e:
                        print(f'Необходимо подождать {e.seconds} секунд прежде чем отправить следующее сообщение в {target_chat.title}.')
                        continue  # Переход к следующему чату
                    except errors.ForbiddenError:
                        try:
                            await client.forward_messages(target_chat, alternative_message_id, source_chat)
                            print(f'Альтернативное сообщение с ID {alternative_message_id} успешно отправлено в {target_chat.title}.')
                        except Exception as e:
                            print(f'Ошибка при отправке альтернативного сообщения в {target_chat.title}: {e}')
                    except errors.ChannelPrivateError:
                        print(f'Канал или чат {target_chat_url} является приватным или нет доступа.')
                        continue  # Переход к следующему чату
                except Exception as e:
                    print(f'Ошибка при доступе к каналу или чату {target_chat_url}: {e}')
